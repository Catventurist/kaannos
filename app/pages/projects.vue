...
import type { Collections, PageCollections, ProEnCollectionItem, ProFiCollectionItem, ProjectsEnCollectionItem, ProjectsFiCollectionItem } from '@nuxt/content'
...
const { data: projects } = await useAsyncData('projects-' + slug.value, async () => {
  const content = await queryCollection(('projects_' + locale.value) as keyof Collections).all()
  if (!content && locale.value !== 'en') {
    return await queryCollection('projects_en').all()
  }
  return content as ProjectsEnCollectionItem[] | ProjectsFiCollectionItem[]
}, {
  watch: [locale]
})
/*
