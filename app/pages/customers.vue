...
useSeoMeta({
  title: $t('header.customers.title'),
  description: $t('header.customers.description')
})
  ...
  return [
    ...
    },
    {
      ...
      onSelect() {
        ...
        toast.add({
          title: $t('customers.copedclip.title'),
          description: $t('customers.copedclip.description')
        })
      }
    },
    {
    ...
    {
      label: $t('customers.details'),
      icon: appConfig.ui.icons.list
    },
    {
      label: $t('customers.payments'),
      icon: appConfig.ui.icons.wallet
    },
    {
      type: 'separator'
    },
    {
      label: $t('customers.delete.label'),
...
const columns: TableColumn<User>[] = [
  {
    ...
    header: ({ table }) =>
      h(UCheckbox, {
        ...
        'ariaLabel': $t('customers.select.all')
      }),
    cell: ({ row }) =>
      h(UCheckbox, {
        'modelValue': row.getIsSelected(),
        'onUpdate:modelValue': (value: boolean | 'indeterminate') => row.toggleSelected(!!value),
        'ariaLabel': $t('customers.select.row')
      })
  },
  {
    accessorKey: 'id',
  ...
  },
  {
    ...
    header: ({ column }) => {
      ...
      return h(UButton, {
        ...
        icon: isSorted
          ? isSorted === 'asc'
            ? appConfig.ui.icons.arrowUpNarrow
            : appConfig.ui.icons.arrowDownWide
          : appConfig.ui.icons.arrowUpDown,
        class: '-mx-2.5',
        ...
        <div class="flex flex-wrap items-center gap-1.5">
          <CustomersDeleteModal :count="table?.tableApi?.getFilteredSelectedRowModel().rows.length">
            <UButton
              ...
              :label="$t('customers.delete.button')"
              color="error"
              variant="subtle"
              :icon="appConfig.ui.icons.trash"
            >
          ...
          <USelect
            ...
            :placeholder="$t('customers.status.filter')"
            class="min-w-28"
          />
          <UDropdownMenu
            :items="
          ...
          >
            <UButton
              ...
              color="neutral"
              variant="outline"
              :trailing-icon="appConfig.ui.icons.settings2"
            />
          </UDropdownMenu>
        </div>
      </div>
