...
<template>
  <div ref="containerRef" class="relative w-[500px]" :class="props.class.trim()">
    <div
      ref="listRef"
      :class="`max-h-[400px] overflow-y-auto p-4 ${displayScrollbar
        ? '[&::-webkit-scrollbar]:w-[8px] [&::-webkit-scrollbar-track]:bg-muted [&::-webkit-scrollbar-thumb]:bg-[#222] [&::-webkit-scrollbar-thumb]:rounded-[4px]'
        : 'scrollbar-hide'}`"
      :style="{
    ...
    >
      ...
      >
        <div :class="`p-4 bg-muted rounded-lg ${selectedIndex === index ? 'bg-muted' : ''} ${itemClass}`">
          <p class="text-default m-0">
            {{ item }}
          </p>
        </div>
      </Motion>
    ...
    <div
      v-if="showGradients"
      class="absolute top-0 left-0 right-0 h-[50px] bg-linear-to-b from-muted to-transparent pointer-events-none transition-opacity duration-300 ease"
      :style="{ opacity: topGradientOpacity }"
    />
    <div
      v-if="showGradients"
      class="absolute bottom-0 left-0 right-0 h-[100px] bg-linear-to-t from-muted to-transparent pointer-events-none transition-opacity duration-300 ease"
