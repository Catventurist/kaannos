...
const flipUnitVariants = tv({
  ...
  variants: {
    ...
    variant: {
      default: 'bg-primary text-inverted',
      secondary: 'bg-info text-inverted',
      destructive: 'bg-dimmed text-dimmed',
      outline: 'border border-input bg-default text-default',
...
const commonCardStyle = 'absolute inset-x-0 overflow-hidden h-1/2 bg-inherit text-inherit will-change-transform'
    ...
    return () =>
      h('div', { class: unitClass.value, style: preserve3d }, [
        ...
        h(
          'div',
          {
            class: `${commonCardStyle} z-20 origin-bottom rounded-t-lg ${flipping.value ? 'animate-flip-top' : ''}`,
        ...
        h(
          'div',
          {
            class: `${commonCardStyle} z-10 origin-top rounded-b-lg translate-y-full ${flipping.value ? 'animate-flip-bottom' : ''}`,
            style: { transform: 'rotateX(90deg)', ...preserve3d, ...backfaceHidden }
          },
          [h(DigitSpan, { position: 'bottom' }, { default: () => localProps.digit })]
        ),
        h('div', {
          class: 'absolute top-1/2 left-0 w-full h-px -translate-y-1/2 bg-default/50 z-30'
        })
      ])
  }
})
