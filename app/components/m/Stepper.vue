...
<template>
  <div class="flex justify-center items-center size-full" v-bind="$attrs">
    <div
      :class="`w-full max-w-md p-8 rounded-4xl shadow-sm shadow-primary ${stepCircleContainerClassName}`"
      ...
      >
        <template v-for="(_, index) in stepsArray" :key="index + 1">
          <div
            ...
            :class="[
              'relative outline-none flex size-8 items-center justify-center rounded-full font-semibold',
          ...
          >
            <svg
              ...
              class="size-4 text-default stroke-default"
              fill="none"
              stroke="currentColor"
              :stroke-width="2"
              viewBox="0 0 24 24"
            ...
            <div v-else-if="getStepStatus(index + 1) === 'active'" class="size-3 rounded-full" />
          ...
          <div
            ...
            class="relative ml-2 mr-2 h-0.5 flex-1 overflow-hidden rounded bg-muted"
          >
            <Motion
              as="div"
      ...
      <div v-if="!isCompleted" :class="`w-full ${footerClassName}`">
        <div :class="`flex w-full ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`">
          <button
            ...
            :class="`text-muted bg-transparent cursor-pointer transition-all duration-350 rounded px-2 py-1 border-none hover:text-default ${currentStep === 1 ? 'opacity-50 cursor-not-allowed' : ''}`"
          ...
          <button
            ...
            :class="`border-none bg-success transition-all duration-350 flex items-center justify-center rounded-full text-default font-medium tracking-tight px-3.5 py-1.5 cursor-pointer hover:bg-info disabled:opacity-50 disabled:cursor-not-allowed`"
