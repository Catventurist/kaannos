...
const emit = defineEmits<{
  (e: 'update:modelValue' | 'change', value: WheelPickerValue): void
}>()
...
const internalValue = ref<WheelPickerValue>(
  props.modelValue ?? props.defaultValue ?? props.options[0]?.value as WheelPickerValue
)
...
const renderItems = computed(() => {
  ...
  if (props.infinite) {
    for (let i = 0; i < quarterCount.value; ++i) {
      ...
      items.unshift({
        option: preOption as WheelPickerOption,
        index: prependIndex,
      ...
      items.push({
        option: postOption as WheelPickerOption,
        index: appendIndex,
...
const renderHighlightItems = computed(() => {
  ...
  if (props.infinite && displayOptions.value.length > 0) {
    ...
    items.unshift({ option: lastItem as WheelPickerOption, key: 'infinite-start' as never })
    items.push({ option: firstItem as WheelPickerOption, key: 'infinite-end' as never })
  }
  ...
  >
    <ul ref="wheelItemsRef" class="rwp-options">
      <li
        ...
        class="rwp-option-item text-muted"
    ...
    <div class="rwp-highlight-wrapper" :style="{ height: `${itemHeight}px`, lineHeight: `${itemHeight}px` }">
      <ul ref="highlightListRef" class="rwp-highlight-list" :style="{ top: infinite ? `${-itemHeight}px` : undefined }">
        <li
          v-for="item in renderHighlightItems"
          :key="item.key"
          class="rwp-highlight-item text-default"
