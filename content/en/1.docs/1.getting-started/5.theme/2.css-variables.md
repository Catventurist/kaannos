---
title: CSS Variables
description: Muokkaus uses CSS variables as design tokens for flexible, consistent theming with built-in light and dark mode support.
navigation.icon: 'i-lucide-swatch-book'
---

## Colors

Muokkaus provides Tailwind CSS utility classes for each [semantic color](/docs/getting-started/theme/design-system#semantic-colors) you define, allowing you to use class names like `text-error` or `bg-success`:
...
Muokkaus provides Tailwind CSS utility classes for text colors, allowing you to use class names like `text-dimmed` or `text-muted`:
...
Muokkaus provides Tailwind CSS utility classes for background colors, allowing you to use class names like `bg-default` or `bg-muted`:
...
Muokkaus provides Tailwind CSS utility classes for border colors, allowing you to use class names like `border-default` or `border-muted`:
...
Muokkaus overrides Tailwind CSS's default `rounded-*` utilities with a unified border radius system, allowing you to use regular [border radius utilities](https://tailwindcss.com/docs/border-radius) like `rounded-xs` or `rounded-2xl`:
...
Muokkaus provides a `--ui-container` CSS variable that controls the maximum width of the [Container](/docs/components/container) component.
...
Muokkaus provides a `--ui-header-height` CSS variable that controls the height of the [Header](/docs/components/header) component.
...
Muokkaus applies default classes on the `<body>`{lang="html"} element of your app for consistent theming across light and dark modes:
